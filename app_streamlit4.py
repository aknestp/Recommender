...
def show_product_popup(product_data, score=None):
    ...
    with c2:
        rating_val = product_data.get('Rating', 0)
    ...
    exclude_cols = ['Name', 'Brand', 'Category', 'Rating', 'ReviewCount', 'Description', 'ImageURL', 'final_score', 'Name_norm', 'similarity', 'rating_norm', 'review_norm']
...
def render_product_card(row, full_df=None):
    ...
    with st.container(border=True):
        ...
        img_url = str(row['ImageURL']).split('|')[0].strip() if pd.notna(row['ImageURL']) else None
        ...
        st.markdown(f"""
            <div class="product-title" title="{row['Name']}">
                {row['Name']}
            </div>
        """, unsafe_allow_html=True)
        ...
        # 4. TOMBOL POP-UP
        if st.button("Lihat Selengkapnya", key=f"btn_{row.name}", use_container_width=True):
...
def page_recommender(df, recommender, llm_tools, metrics):
...
def main():
    ...
    if st.session_state["current_page"] == "home":
        ...
        page_recommender(df, recommender, llm_tools, metrics)
