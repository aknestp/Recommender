...
from src.rekom import CollaborativeFilteringRecommender  # --- INTEGRASI: Tambahkan import CF ---
...
# --- INTEGRASI DARI KODE SEBELUMNYA: Fungsi Bantu ---
def add_to_cart(pid):
    if 'cart' not in st.session_state:
        st.session_state.cart = []
    if pid not in st.session_state.cart:
        st.session_state.cart.append(pid)
        st.success(f"Produk {pid} ditambahkan ke keranjang!")
    else:
        st.warning(f"Produk {pid} sudah ada di keranjang!")

def display_grid(products, title, full_df=None, prefix=""):
    """Menampilkan grid produk menggunakan render_product_card yang sudah ada."""
    st.subheader(title)
    cols = st.columns(5)  # Grid 5 kolom untuk konsistensi
...
def show_product_popup(product_data, score=None):
    ...
    with c2:
        # --- PERBAIKAN: Handle kolom rating dengan fallback ---
    ...
    with col_cart:
        if st.button("🛒 Add to Cart", use_container_width=True, key=f"cart_{product_data.get('Name')}"):
            add_to_cart(product_data.get('ProdID', product_data.get('Name')))  # --- INTEGRASI: Gunakan add_to_cart ---
            st.toast("Produk berhasil ditambahkan ke Keranjang! 🛒", icon="✅")
    
    with col_buy:
        # Tombol Buy Now (Primary)
        if st.button("🛍️ Buy Now", type="primary", use_container_width=True, key=f"buy_{product_data.get('Name')}"):
    ...
    exclude_cols = ['Name', 'Brand', 'Category', 'Rating', 'ReviewCount', 'Description', 'ImageURL', 'final_score', 'Name_norm', 'similarity', 'rating_norm', 'review_norm', 'average_rating']  # --- PERBAIKAN: Tambahkan 'average_rating' ke exclude ---
...
def render_product_card(row, full_df=None, prefix=""):
    ...
    with st.container(border=True):
        ...
        # --- PERBAIKAN: Handle kolom rating dengan fallback ---
        rating_val = row.get('Rating', row.get('average_rating', 0))  # Prioritas: Rating > average_rating > 0
        ...
        # 4. TOMBOL POP-UP
        # --- PERBAIKAN: Gunakan prefix untuk key unik ---
...
def initialize_system():
    ...
    try:
        ...
        if df.empty:
            return None, None, None, None, None  # --- INTEGRASI: Tambahkan return untuk CF ---
        ...
        # --- INTEGRASI: Inisialisasi CF Recommender ---
        cf_recommender = CollaborativeFilteringRecommender(data_path=DATA_FILE_PATH, num_users=500, random_seed=42)
        
        return df, recommender, llm_tools, metrics, cf_recommender  # --- INTEGRASI: Return CF ---
...
def page_recommender(df, recommender, llm_tools, metrics, cf_recommender):  # --- INTEGRASI: Tambahkan cf_recommender ---
    # --- 1. HEADER SECTION (Search & Controls) ---
    st.title("🛍️ Pencarian & Rekomendasi")
    
    # Container Header dengan MARKER ID agar bisa ditarget CSS
    with st.container(border=True):
        st.markdown('<span id="header-marker"></span>', unsafe_allow_html=True) # !!! MARKER PENTING !!!
        
        # Layout 6 Kolom: [Back] [Logo] [Search] [Num] [SearchBtn] [EvalBtn]
        ...
        # 2. Logo Teks (PUTIH karena Header Biru)
        with c_logo:
            st.markdown("""
                <div style='
                    font-size: 24px; 
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset Cache / Refresh"):  # --- INTEGRASI: Tombol Refresh ---
            st.cache_resource.clear()
            st.rerun()
        
        # --- INTEGRASI: Keranjang Belanja ---
...
def page_home(df, cf_recommender):  # --- INTEGRASI: Tambahkan cf_recommender ---
    # --- HEADER BARU ---
    # Header Biru dengan MARKER ID
    with st.container(border=True):
        st.markdown('<span id="header-marker"></span>', unsafe_allow_html=True) # !!! MARKER PENTING !!!
    ...
    if search_input:
        ...
        st.rerun()
    st.markdown("---")
    # --- PERBAIKAN: Urutan Baru - Produk Terlaris (Atas) ---
    if cf_recommender:
    ...
    for idx, (index, row) in enumerate(display_df.iterrows()):
        with cols[idx % 5]:
            render_product_card(row, full_df=df)  # --- PERBAIKAN: Tanpa prefix ---
    
    # --- PERBAIKAN: Rekomendasi Produk (Bawah) ---
    if cf_recommender:
...
def main():
    ...
    st.markdown("""
        ...
        /* Semua container border (defaultnya kartu produk) berwarna Abu-abu */
        ...
        /* Tombol di dalam header (container biru) harus Putih */
    ...
    df, recommender, llm_tools, metrics, cf_recommender = initialize_system()  # --- INTEGRASI: Update return values ---
    ...
    if st.session_state["current_page"] == "home":
        page_home(df, cf_recommender)  # --- INTEGRASI: Pass cf_recommender ---
    elif st.session_state["current_page"] == "recommender":
        page_recommender(df, recommender, llm_tools, metrics, cf_recommender)  # --- INTEGRASI: Pass cf_recommender ---
